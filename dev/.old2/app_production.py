    ...
    for course_id, course in json_data.items():
        ...
        for section in curriculum.get("sections", []):
            ...
            for item in section.get("items", []):
                ...
                # Combine title and status in one cell with emojis
...
def generate_individual_markdowns(json_data, statuses):
    """Generate a dictionary of course_id: markdown for individual courses."""
    md_files = {}
    for course_id, course in json_data.items():
        instructor = course.get("instructor", "Unknown Instructor")
        curriculum = course.get("curriculum_context", {}).get("data", {})
...
if st.session_state["json_data"] is not None:
    ...
    st.sidebar.download_button(
        ...
        file_name="selected_course_output.md",
        mime="text/markdown",
    )
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for course_id, markdown_text in md_files.items():
            # Use the course_id in the filename, or you can add the course title if desired.
            zip_file.writestr(f"{course_id}.md", markdown_text)
    zip_buffer.seek(0)
    st.sidebar.download_button(
