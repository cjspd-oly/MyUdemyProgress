...
if st.session_state["json_data"] is not None:
    ...
    for course_id, course in json_data.items():
        ...
        for section in curriculum.get("sections", []):
            ...
            for item in section.get("items", []):
                ...
                with col2:
                    # Unique key used for selectbox to avoid duplicates; append a constant suffix to ensure uniqueness.
    ...
    # Markdown download button always visible in the sidebar once data is loaded.
