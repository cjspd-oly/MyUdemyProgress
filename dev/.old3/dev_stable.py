...
sections, lectures, and progress analytics. It supports:
  ...
  - Additional features: color-coded badges, lecture filters, favorite courses,
    lazy loading, and auto-collapsing of completed sections.
...
def get_ui_status(value):
    ...
    elif value in universal_status_options:
        return value
    else:
...
def render_sidebar():
    ...
    autosave_enabled = st.sidebar.checkbox("💾 Autosave", value=False, key="autosave_checkbox")
    ...
    return autosave_enabled, preload_enabled, filter_option
...
def render_course_details(selected_course_id, json_data, statuses, status_options, filter_option):
    ...
    for sec in curriculum.get("sections", []):
        ...
        default_expanded = True if filter_option == "All" or len(visible_items) > 0 else False
...
def main():
    ...
    autosave_enabled, preload_enabled, filter_option = render_sidebar()
    ...
    if st.session_state["json_data"] is not None:
        # Merge statuses from the course JSON if present
