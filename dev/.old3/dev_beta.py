...
def get_ui_status(value):
    ...
    """
    if value in universal_status_options:
        return value
    elif value in status_mapping:
...
def render_sidebar():
    """Render sidebar options for autosave, preload, course selection, and filters."""
...
def render_course_details(selected_course_id, json_data, statuses, status_options, filter_option):
    ...
    for sec in curriculum.get("sections", []):
        ...
        default_expanded = expand_all or (True if filter_option == "All" or len(visible_items) > 0 else False)
