...
def render_course_details(selected_course_id, json_data, statuses, status_options):
    ...
    # Define color-coded badges
    status_badges = {
        "✅ Done": '<span style="background-color: #4CAF50; color: white; padding: 2px 6px; border-radius: 5px;">✅ Done</span>',
        "⏳ In Progress": '<span style="background-color: #FFC107; color: black; padding: 2px 6px; border-radius: 5px;">⏳ In Progress</span>',
        "❌ Not Done": '<span style="background-color: #F44336; color: white; padding: 2px 6px; border-radius: 5px;">❌ Not Done</span>',
        "⭐ Important": '<span style="background-color: #FF9800; color: white; padding: 2px 6px; border-radius: 5px;">⭐ Important</span>',
        "⏭ Skip": '<span style="background-color: #9E9E9E; color: white; padding: 2px 6px; border-radius: 5px;">⏭ Skip</span>',
        "🚫 Ignore": '<span style="background-color: #607D8B; color: white; padding: 2px 6px; border-radius: 5px;">🚫 Ignore</span>',
    }
    ...
    for sec in curriculum.get("sections", []):
        ...
        # **MERGED SECTION HEADER WITH COLOR CODED BADGES**
        section_header = (
            f"<b>📂 {sec_title}</b>  <br>"
            f"✅ {done_count}/{lect_count} lectures • ⏱ {length}"
        )

        with st.expander(section_header, expanded=(done_count < lect_count)):
            ...
            for item in sec.get("items", []):
                ...
                # Display color-coded badge for status
                st.markdown(
                    f'<p style="text-align:left">{status_badges.get(statuses[key], statuses[key])}</p>',
                    unsafe_allow_html=True,
                )
